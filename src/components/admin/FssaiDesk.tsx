...
import {
  Utensils, Download, FileSpreadsheet, FileText, Loader2, RefreshCcw, Search, Upload,
} from "lucide-react";
...
import {
  adminListFssaiApplications, adminGetFssaiApplication,
  adminUpdateFssaiStatus, adminUploadFssaiCertificate,
  getFssaiDocumentUrl, getFssaiCertificateUrl,
} from "@/lib/fssai.functions";
import {
  FSSAI_STATUSES, FSSAI_STATUS_LABEL, FSSAI_STATUS_TONE, type FssaiStatus,
} from "@/lib/fssai.shared";
import { supabase } from "@/integrations/supabase/client";
import { formatINR } from "@/lib/queries";

const CARDS: Array<{ key: FssaiStatus; label: string; tone: string }> = [
...
export function FssaiDesk() {
  const queryClient = useQueryClient();
  const [statusFilter, setStatusFilter] = useState<string>("all");
  const [search, setSearch] = useState("");
  const [activeId, setActiveId] = useState<string | null>(null);

  const listFn = useServerFn(adminListFssaiApplications);
  const list = useQuery({
    queryKey: ["admin-fssai", statusFilter, search],
    queryFn: () => listFn({
      data: {
        status: statusFilter === "all" ? undefined : (statusFilter as FssaiStatus),
        q: search || undefined,
      },
    }),
  });
  ...
  useEffect(() => {
    const channel = supabase.channel("admin-fssai")
      .on("postgres_changes", { event: "*", schema: "public", table: "fssai_applications" },
        () => queryClient.invalidateQueries({ queryKey: ["admin-fssai"] }))
      .subscribe();
  ...
  async function exportExcel() {
    ...
    const ws = XLSX.utils.json_to_sheet(rows.map((r: any) => ({
      Application: r.application_no, Business: r.business_name, Applicant: r.applicant_name,
      License: r.license_type, Mobile: r.mobile, Aadhaar: r.aadhaar_number, PAN: r.pan_number,
      District: r.business_district, State: r.business_state,
      Status: FSSAI_STATUS_LABEL[r.status as FssaiStatus],
      Charged: Number(r.total_charged || 0),
      Created: new Date(r.created_at).toLocaleString("en-IN"),
    })));
    XLSX.utils.book_append_sheet(wb, ws, "FSSAI");
    XLSX.writeFile(wb, `fssai-applications-${Date.now()}.xlsx`);
  }
  ...
  async function exportPDF() {
    ...
    doc.text("Food License (FSSAI) Applications", 14, 14);
    autoTable(doc, {
      startY: 18,
      head: [["App No", "Business", "Applicant", "License", "Mobile", "District", "Status", "Charged", "Created"]],
      body: rows.map((r: any) => [
        r.application_no, r.business_name || "", r.applicant_name || "", r.license_type || "",
        r.mobile || "", r.business_district || "",
    ...
    doc.save(`fssai-applications-${Date.now()}.pdf`);
  }
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-2xl font-bold">Food License (FSSAI) Desk</h1>
          <p className="text-[13px] text-muted-foreground">Process, verify and issue FSSAI Food License registrations.</p>
        </div>
        <div className="flex gap-2">
          ...
          <Button variant="outline" onClick={() => queryClient.invalidateQueries({ queryKey: ["admin-fssai"] })}>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            ...
            <SelectContent>
              ...
              {FSSAI_STATUSES.map((s) => <SelectItem key={s} value={s}>{FSSAI_STATUS_LABEL[s]}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
      </Card>
      ...
      <Sheet open={!!activeId} onOpenChange={(o) => !o && setActiveId(null)}>
        <SheetContent side="right" className="w-full max-w-3xl overflow-y-auto sm:max-w-3xl">
          <SheetHeader>
            <SheetTitle className="flex items-center gap-2"><Utensils className="h-5 w-5" /> Application</SheetTitle>
          </SheetHeader>
...
function DetailPanel({ id }: { id: string }) {
  const queryClient = useQueryClient();
  const getOne = useServerFn(adminGetFssaiApplication);
  const detail = useQuery({ queryKey: ["admin-fssai-detail", id], queryFn: () => getOne({ data: { id } }) });
  const app = detail.data?.app;
  const docs = detail.data?.docs ?? [];
  const events = detail.data?.events ?? [];

  const [status, setStatus] = useState<FssaiStatus>("submitted");
  const [remarks, setRemarks] = useState("");

  useEffect(() => {
    if (app?.status) setStatus(app.status as FssaiStatus);
    setRemarks(app?.remarks ?? "");
  }, [app?.id]);

  const updateFn = useServerFn(adminUpdateFssaiStatus);
  const updateMutation = useMutation({
    mutationFn: () => updateFn({ data: { applicationId: id, status, remarks } }),
    onSuccess: () => {
      toast.success("Status updated");
      queryClient.invalidateQueries({ queryKey: ["admin-fssai-detail", id] });
      queryClient.invalidateQueries({ queryKey: ["admin-fssai"] });
    },
    onError: (e: any) => toast.error(e?.message || "Update failed"),
  });

  const docUrlFn = useServerFn(getFssaiDocumentUrl);
  const certUrlFn = useServerFn(getFssaiCertificateUrl);
  const certUploadFn = useServerFn(adminUploadFssaiCertificate);

  async function openDoc(docId: string) {
    try { const { url } = await docUrlFn({ data: { id: docId } }); window.open(url, "_blank", "noopener,noreferrer"); }
    catch (e: any) { toast.error(e?.message || "Could not open"); }
  }
  async function openCert() {
    try { const { url } = await certUrlFn({ data: { applicationId: id } }); window.open(url, "_blank", "noopener,noreferrer"); }
    ...
    <div className="space-y-4 py-2">
      <div className="rounded-lg border border-border bg-muted/30 p-3">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className={FSSAI_STATUS_TONE[app.status as FssaiStatus]}>
            {FSSAI_STATUS_LABEL[app.status as FssaiStatus]}
          </Badge>
        </div>
        ...
        <div className="col-span-2 space-y-3">
          <div className="flex flex-wrap gap-2">
            <div className="min-w-[180px] flex-1">
              ...
              <Select value={status} onValueChange={(v) => setStatus(v as FssaiStatus)}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {FSSAI_STATUSES.filter((s) => s !== "draft").map((s) =>
                    <SelectItem key={s} value={s}>{FSSAI_STATUS_LABEL[s]}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </div>
      ...
      <Section title="Final Certificate">
        <div className="col-span-2">
          <CertSlot label="FSSAI Certificate" existing={!!app.certificate_url} onOpen={openCert} onUpload={uploadCert} />
        </div>
      </Section>
            ...
            <div key={e.id} className="rounded-md border border-border bg-muted/20 px-3 py-2 text-[12.5px]">
              ...
              {(e.from_status || e.to_status) && (
                <div className="text-[11.5px] text-muted-foreground">
                  {e.from_status ? `${FSSAI_STATUS_LABEL[e.from_status as FssaiStatus]} → ` : ""}
                  {e.to_status ? FSSAI_STATUS_LABEL[e.to_status as FssaiStatus] : ""}
                </div>
              )}
