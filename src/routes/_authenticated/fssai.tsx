...
import {
  Utensils, CheckCircle2, FileUp, Info, Loader2, Save, Send, Trash2,
  User as UserIcon, Wallet as WalletIcon, Briefcase, ShieldCheck,
...
import {
  saveFssaiDraft, submitFssaiApplication, uploadFssaiDocument,
  deleteFssaiDocument, getMyFssaiApplication, getFssaiConfig,
} from "@/lib/fssai.functions";
import { useMe, formatINR } from "@/lib/queries";
import {
  FSSAI_BUSINESS_TYPES, GENDERS, FSSAI_DOC_TYPES, FSSAI_FOOD_CATEGORIES,
  FSSAI_LICENSE_TYPES, FSSAI_STATUS_LABEL, FSSAI_STATUS_TONE, type FssaiStatus,
} from "@/lib/fssai.shared";

const searchSchema = z.object({ id: z.string().uuid().optional() });

export const Route = createFileRoute("/_authenticated/fssai")({
  head: () => ({ meta: [{ title: "Food License (FSSAI) Registration — Vighnaharta Solutions" }] }),
  validateSearch: searchSchema,
  component: FssaiPage,
});

type FormState = {
  applicant_name: string; father_name: string; dob: string; gender: string;
  mobile: string; email: string; aadhaar_number: string; pan_number: string;
  business_name: string; business_type: string; food_category: string;
  license_type: string; validity_years: string;
...
const EMPTY: FormState = {
  ...
  mobile: "", email: "", aadhaar_number: "", pan_number: "",
  business_name: "", business_type: "", food_category: "",
  license_type: "Basic", validity_years: "1",
  business_address: "", business_city: "", business_district: "", business_state: "", business_pincode: "",
};

function toPayload(f: FormState) {
  return {
    applicant_name: f.applicant_name.trim() || undefined,
    ...
    food_category: f.food_category || undefined,
    license_type: f.license_type || undefined,
    validity_years: f.validity_years ? Number(f.validity_years) : undefined,
...
function fromApp(a: any): FormState {
  return {
    applicant_name: a?.applicant_name ?? "", father_name: a?.father_name ?? "",
    ...
    license_type: a?.license_type ?? "Basic",
    validity_years: String(a?.validity_years ?? 1),
    business_address: a?.business_address ?? "",
    business_city: a?.business_city ?? "", business_district: a?.business_district ?? "",
    business_state: a?.business_state ?? "", business_pincode: a?.business_pincode ?? "",
  };
}
...
function FssaiPage() {
  ...
  const { data: me } = useMe();
  const cfgFn = useServerFn(getFssaiConfig);
  const cfg = useQuery({ queryKey: ["fssai-cfg"], queryFn: () => cfgFn() });
  ...
  const fee = useMemo(() => {
    ...
    if (!c) return 699;
    if (form.license_type === "State") return Number(c.state_price);
    if (form.license_type === "Central") return Number(c.central_price);
    return Number(c.basic_price);
  }, [cfg.data, form.license_type]);

  const getOne = useServerFn(getMyFssaiApplication);
  const detail = useQuery({
    queryKey: ["fssai-app", appId],
  ...
  const saveFn = useServerFn(saveFssaiDraft);
  const saveMutation = useMutation({
    ...
    onSuccess: (res) => {
      ...
      if (!appId && res.id) {
        setAppId(res.id);
        navigate({ to: "/fssai", search: { id: res.id } });
      }
      queryClient.invalidateQueries({ queryKey: ["fssai-app", res.id] });
      queryClient.invalidateQueries({ queryKey: ["my-fssai"] });
    },
    onError: (e: any) => toast.error(e?.message || "Could not save"),
  });

  const submitFn = useServerFn(submitFssaiApplication);
  const submitMutation = useMutation({
    mutationFn: () => submitFn({ data: { id: appId! } }),
    onSuccess: (res) => {
      toast.success(`Submitted! Application ${res.application_no}. ${formatINR(res.charged)} debited.`);
      queryClient.invalidateQueries();
      navigate({ to: "/fssai-applications" });
    },
    ...
    <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[12.5px] text-primary-foreground/70">
                Basic, State or Central license. Wallet debit: <strong>{formatINR(fee)}</strong> on final submit.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            {detail.data?.app?.status && (
              <Badge className={FSSAI_STATUS_TONE[detail.data.app.status as FssaiStatus]}>
                {FSSAI_STATUS_LABEL[detail.data.app.status as FssaiStatus]}
              </Badge>
            )}
            <Link to="/fssai-applications">
      ...
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList className="flex w-full flex-wrap gap-1 bg-muted/40 p-1">
          ...
          <TabsTrigger value="license"><ShieldCheck className="mr-1.5 h-3.5 w-3.5" />License</TabsTrigger>
        ...
        <TabsContent value="instructions" className="mt-4">
          <Card className="p-6">
            ...
            <ul className="mt-3 space-y-2 text-[13.5px] text-foreground/85">
              <li className="flex gap-2"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" />Choose the correct license type based on annual turnover (Basic / State / Central).</li>
              <li className="flex gap-2"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" />Provide accurate food business details, food category and validity.</li>
              <li className="flex gap-2"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" />Keep KYC, electricity bill, rent agreement and shop photo ready.</li>
              ...
              <li className="flex gap-2"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" />Service fee <strong>{formatINR(fee)}</strong> is debited from your wallet on final submission.</li>
              <li className="flex gap-2"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" />Save as draft any time and continue later.</li>
            </ul>
        ...
        <TabsContent value="personal" className="mt-4">
          <Card className="grid grid-cols-1 gap-4 p-6 md:grid-cols-2">
            <Field label="Applicant Name *"><Input value={form.applicant_name} disabled={!!isLocked} onChange={(e) => setForm({ ...form, applicant_name: e.target.value })} /></Field>
        ...
        <TabsContent value="business" className="mt-4">
          <Card className="grid grid-cols-1 gap-4 p-6 md:grid-cols-2">
            <Field label="Business Name *"><Input value={form.business_name} disabled={!!isLocked} onChange={(e) => setForm({ ...form, business_name: e.target.value })} /></Field>
            <Field label="Business Type *">
              <Select value={form.business_type} disabled={!!isLocked} onValueChange={(v) => setForm({ ...form, business_type: v })}>
                <SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger>
                <SelectContent>{FSSAI_BUSINESS_TYPES.map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
              </Select>
            </Field>
            <Field label="Food Business Category *" className="md:col-span-2">
              <Select value={form.food_category} disabled={!!isLocked} onValueChange={(v) => setForm({ ...form, food_category: v })}>
                <SelectTrigger><SelectValue placeholder="Select category" /></SelectTrigger>
                <SelectContent>{FSSAI_FOOD_CATEGORIES.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
              </Select>
            </Field>
            <Field label="Business Address *" className="md:col-span-2"><Textarea rows={2} value={form.business_address} disabled={!!isLocked} onChange={(e) => setForm({ ...form, business_address: e.target.value })} /></Field>
        ...
        <TabsContent value="license" className="mt-4">
          <Card className="grid grid-cols-1 gap-4 p-6 md:grid-cols-2">
            <Field label="License Type *">
              <Select value={form.license_type} disabled={!!isLocked} onValueChange={(v) => setForm({ ...form, license_type: v })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>{FSSAI_LICENSE_TYPES.map((t) => <SelectItem key={t.id} value={t.id}>{t.label}</SelectItem>)}</SelectContent>
              </Select>
            </Field>
            <Field label="Validity (Years) *">
              <Select value={form.validity_years} disabled={!!isLocked} onValueChange={(v) => setForm({ ...form, validity_years: v })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>{[1, 2, 3, 4, 5].map((n) => <SelectItem key={n} value={String(n)}>{n} Year{n > 1 ? "s" : ""}</SelectItem>)}</SelectContent>
              </Select>
            </Field>
            <div className="md:col-span-2 rounded-lg border border-border bg-muted/30 p-4 text-[13px]">
              <div className="font-semibold text-foreground/90">Selected: {form.license_type} License · {form.validity_years} year(s)</div>
              <div className="mt-1 text-muted-foreground">Service fee for this combination: <strong className="tabular-nums">{formatINR(fee)}</strong></div>
            </div>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="documents" className="mt-4">
          {!appId ? (
            ...
            <DocumentsSection applicationId={appId} isLocked={!!isLocked}
              ...
              onChange={() => queryClient.invalidateQueries({ queryKey: ["fssai-app", appId] })} />
          )}
        </TabsContent>
        ...
        <TabsContent value="review" className="mt-4">
          <Card className="space-y-4 p-6">
            ...
            <p className="text-[13px] text-muted-foreground">
              On final submission, <strong>{formatINR(fee)}</strong> is debited from your wallet and a unique Application ID is generated.
            </p>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <Summary label="Applicant" value={form.applicant_name || "—"} />
              ...
              <Summary label="Food Category" value={form.food_category || "—"} />
              <Summary label="License" value={`${form.license_type} · ${form.validity_years}y`} />
              <Summary label="Address" value={`${form.business_city || "—"}, ${form.business_district || "—"}, ${form.business_state || "—"} - ${form.business_pincode || "—"}`} />
              <Summary label="Documents uploaded" value={`${detail.data?.docs?.length ?? 0} / ${FSSAI_DOC_TYPES.filter(d => d.required).length} required`} />
            </div>
      ...
      <div className="sticky bottom-3 z-10 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border bg-card/95 p-3 shadow-elegant backdrop-blur">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {(() => {
            const STEPS = ["instructions","personal","business","license","documents","review"];
...
function DocumentsSection({
  ...
  const uploadFn = useServerFn(uploadFssaiDocument);
  const deleteFn = useServerFn(deleteFssaiDocument);
